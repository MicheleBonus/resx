...
def test_map_pdb_missing_fields(client):
    ...
    assert 'pdb_id' in response.get_json()['message']
...
def test_map_pdb_invalid_numbers(client):
    ...
    assert response.status_code == 400
    assert 'window' in response.get_json()['message']
...
def test_map_pdb_window_too_large(client):
    ...
    assert response.status_code == 400
    assert response.get_json()['message'] == 'window must be an integer between 0 and 10'
...
def test_map_pdb_insertion_code_length(client):
    ...
    assert response.status_code == 400
    assert 'insertion_code' in response.get_json()['message']
...
def test_map_pdb_not_found(client, mock_polars):
    ...
    assert 'No mapping found' in response.get_json()['message']


def test_map_uniprot_missing_fields(client):
    response = client.post('/map/uniprot', data={'residue': '5'})
    assert response.status_code == 400
    assert 'uniprot_id' in response.get_json()['message']
